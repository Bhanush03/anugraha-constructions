...
type ApiErrorLike = { error?: string; message?: string; issues?: unknown };
function getApiErrorMessage(err: unknown): string {
  if (!err) return "Unknown error";
  if (typeof err === "string") return err;
  const maybe = err as any;
  if (maybe?.message) return String(maybe.message);
  if (maybe?.error || maybe?.issues) return [maybe.error, maybe.message].filter(Boolean).join(" - ");
  return err instanceof Error ? err.message : JSON.stringify(err);
}
function formatApiErrorPayload(payload: ApiErrorLike): string {
  const parts: string[] = [];
  if (payload.error) parts.push(String(payload.error));
  if (payload.message) parts.push(String(payload.message));
  if (payload.issues) parts.push(`issues: ${JSON.stringify(payload.issues)}`);
...
const adminNav = [
  ...
  { label: "Settings", href: "/settings", icon: ImageIcon }
];
      ...
      <div className="flex min-h-screen">
        <aside className="hidden w-80 flex-col border-r border-white/10 bg-[#071b34] px-5 py-6 lg:flex">
          ...
          <div className="mt-auto space-y-3">
            ...
            <button onClick={logoutAdmin} className="flex w-full items-center justify-between rounded-2xl bg-rose-500/10 px-4 py-3 text-sm font-semibold text-rose-200">
        ...
        <main className="flex-1 overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(10,102,255,0.16),transparent_25%),radial-gradient(circle_at_top_right,rgba(255,255,255,0.08),transparent_18%),#071b34]">
          <div className="border-b border-white/10 bg-white/5 px-4 py-4 lg:hidden">
            <div className="flex items-center justify-between">
              ...
              <button onClick={logoutAdmin} className="rounded-full border border-white/10 px-3 py-2 text-sm">Log Out</button>
            </div>
          </div>
...
function DashboardPage() {
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      alert("Failed to save to server. Is the API reachable?");
    }
  };
      ...
      <Modal open={editOpen} title="Edit Dashboard Stats" description="Update public-facing stats shown on the homepage." onClose={() => setEditOpen(false)}>
        <form className="grid gap-4">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={saveToServer} className="bg-primary text-white" type="button">Save To Server</Button>
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const formatted = formatApiErrorPayload(payload);
        message = formatted ? `${message}: ${formatted}` : message;
      } catch {
        // ignore
      }
      alert(message);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-[20px] border-white/10 bg-white/5 text-white shadow-2xl">
        ...
        <CardContent>
          <div className="grid gap-3">
            <EditableSettingRow label="Office Name" value={String(overrides.officeName ?? "Shivakripa building")} placeholder="Shivakripa building" onSave={(nextValue) => update({ officeName: nextValue })} helperText="Shown on the Office card and in the footer presence line." />
            <EditableSettingRow label="Office Address" value={String(overrides.officeAddress ?? "APMC ROAD, near City hospital, Puttur, Karnataka 574201")} placeholder="APMC ROAD, near City hospital, Puttur, Karnataka 574201" onSave={(nextValue) => update({ officeAddress: nextValue })} isTextarea helperText="Shown as the clickable location text on the homepage." />
            <EditableSettingRow label="Google Maps URL" value={String(overrides.mapsUrl ?? "https://maps.app.goo.gl/eFWv8LbsT6YXw1VH8")} placeholder="https://maps.app.goo.gl/..." onSave={(nextValue) => update({ mapsUrl: nextValue })} helperText="Used for the map pin, office link, and Google review card if no review URL is set." />
      ...
      <Card className="rounded-[20px] border-white/10 bg-white/5 text-white shadow-2xl">
        ...
        <CardContent>
          <div className="grid gap-3">
            <EditableSettingRow label="Google Review URL" value={String(overrides.googleReviewUrl ?? "https://maps.app.goo.gl/eFWv8LbsT6YXw1VH8")} placeholder="https://maps.app.goo.gl/..." onSave={(nextValue) => update({ googleReviewUrl: nextValue })} helperText="Shown on the Google Review CTA and footer review card." />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex gap-3">
        ...
        <Button onClick={saveToServer} className="bg-primary text-white">Save To Server</Button>
...
export default function AdminPage() {
  ...
  return (
    <Shell>
      ...
      {path === "/callbacks" && <CallbacksPage />}
      {path === "/team" && <TeamPage />}
      {path === "/services" && <ServicesPage />}
      {path === "/testimonials" && <TestimonialsPage />}
